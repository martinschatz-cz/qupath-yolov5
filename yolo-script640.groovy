...
int w0 = (int) roi.getBoundsWidth()
int h0 = (int) roi.getBoundsHeight()

// 2. Request pixels from QuPath and send tiles to Docker
int maxSize = 640
...
for (int ty = y0; ty < y0 + h0; ty += maxSize) {
    int tileH = Math.min(maxSize, y0 + h0 - ty)
    for (int tx = x0; tx < x0 + w0; tx += maxSize) {
        int tileW = Math.min(maxSize, x0 + w0 - tx)
